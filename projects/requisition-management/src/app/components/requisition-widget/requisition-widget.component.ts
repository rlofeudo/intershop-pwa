...
export class RequisitionWidgetComponent implements OnInit {
  ...
  ngOnInit() {
    const pendingRequisitions$ = this.requisitionFacade
      .buyerPendingRequisitions$()
      .pipe(filter(reqs => reqs.length > 0));
    ...
    this.totalAmountRequisitons$ = pendingRequisitions$.pipe(
      ...
      map(prices =>
        prices?.reduce((curr: Price, acc: Price) => PriceHelper.sum(curr, acc), PriceHelper.empty(prices[0].currency))
      )
    );
